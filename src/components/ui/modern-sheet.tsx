...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      <SheetPrimitive.Close className="absolute right-5 top-5 rounded-full w-8 h-8 flex items-center justify-center bg-marine-50 opacity-70 ring-offset-white transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-marine-500 focus:ring-offset-2 disabled:pointer-events-none">
        <X className="h-4 w-4 text-marine-700" />
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Description
    ref={ref}
    className={cn("text-sm text-marine-500", className)}
    {...props}
  />
));
