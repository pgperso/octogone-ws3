...
// Exclut 'cortex' de la navigation circulaire
export function getNextConcept(currentId: string): ConceptFeature | null {
  // Filtrer pour exclure cortex de la navigation
  const navigationConcepts = concepts.filter(c => c.id !== 'cortex');
  const currentIndex = navigationConcepts.findIndex(c => c.id === currentId);
  if (currentIndex === -1) return null;
  
  // Boucle : si on est à la fin, retourner au début
  const nextIndex = (currentIndex + 1) % navigationConcepts.length;
  return navigationConcepts[nextIndex];
}
...
export function getPreviousConcept(currentId: string): ConceptFeature | null {
  ...
  const previousIndex = currentIndex === 0 ? navigationConcepts.length - 1 : currentIndex - 1;
  return navigationConcepts[previousIndex];
}
